...
3. **Start the 7-day free trial** — one-click approval through Shopify Billing (no payment details collected by Agalaz; trial does not charge).
...
2. Select your store → approve the permission (read_products).
...
3. You are redirected to Shopify's billing approval page → click **Approve**.
4. No charge during the trial. Trial starts for 7 days with 50 renders unlocked.
5. Back on the dashboard you see an amber banner: "Free trial — 7 days remaining".
6. If you DON'T cancel before day 7 → Starter activates automatically, $149 billed via Shopify, 200 renders unlocked for the month.
...
1. In Shopify admin, go to **Settings → Billing → Subscriptions**.
2. Find **Agalaz Virtual Try-On** in the list and click **Cancel**.
3. You keep access until the end of the current billing period.
4. Before day 7 of trial = $0 charged. After day 7 = current period is paid, no future charges.
5. Alternative: uninstall the app from Shopify admin → Apps. This also cancels the subscription automatically.
...
- **Credits not refreshing**: Refresh the dashboard. Credits sync after each render and after each Shopify billing cycle (may take up to 1 minute).
- **Billing question or wrong charge**: Check the subscription status in your Shopify admin under **Settings → Billing → Subscriptions**. All charges appear on your Shopify invoice.
- **Trial not activating**: If you closed Shopify's billing approval page without confirming, the trial did NOT start. Reopen the dashboard and click **Start free trial** again.
